...
to use `data.table` objects then convert afterwards


```r
library(rdhs)

indicators <- as.data.table(dhs_indicators())
...
```r
countries <- as.data.table(dhs_countries())
...
```r
statcomp <- as.data.table(dhs_data(indicatorIds = "AN_ANEM_W_ANY", countryIds = dhscc))
...
<img src="https://github.com/OJWatson/rdhs/raw/development/vignettes/anemia_files/figure-html/unnamed-chunk-5-1.png" style="display: block; margin: auto;" />
...
```r
surveychar <- as.data.table(dhs_surveyCharacteristics())
...
```r
surveys <- as.data.table(dhs_surveys(surveyCharacteristicIds = 41, countryIds = dhscc))
...
```r
datasets <- as.data.table(dhs_datasets(surveyIds = surveys$SurveyId, fileType = "IR", fileFormat="flat"))
...
To get datasets, we set up our log in credentials by providing the file path to a text file where our credentials are saved. `rdhs` will then create a cache in a specified location to save downloaded datasets and metadata. Credentials should be stored in a text file with three lines:
...
The file path to to this credentials file and the desired location for the cache are passed to the `set_dhs_credentials()` function: 
...
set_dhs_credentials(credentials = "credentials_file",
...
<img src="https://github.com/OJWatson/rdhs/raw/development/vignettes/anemia_files/figure-html/unnamed-chunk-20-1.png" style="display: block; margin: auto;" />
