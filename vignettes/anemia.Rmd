...
To get datasets, we establish a `dhsclient` object. The DHS client creates a cache in a specified location to save downloaded datasets and metatdata. The client requires login credentials for accessing datasets from the DHS website, including the email address, password, and project name. Credentials should be stored in a text file with three lines:
...
The file path to to this credentials file and the desired location for the cache are passed to the `client()` function to establish the DHS client.

```{r }
client <- dhs_client(credentials = "~/Documents/Data/DHS/rdhs/credentials",
                     root = "~/Downloads/rdhs_cache")
```
...
```{r }
datasets$path <- client$get_datasets(datasets$FileName)
```
...
```{r }
head(client$survey_questions(datasets$FileName[1], "hemoglobin")[,1:2])
```
...
The results suggest a that anemia prevalence is lower among women with higher education.
