...
handled the downstream functionality of rdhs. As a result the best way to demonstrate rdhs
...
4. Cataloging which surveys you can download for your log in credentials
...
time at which it was last updated, so that every time you use rdhs afterwards, the client will
...
to a) completely delete the client's cache, or b) update any cached surveys or information
in bulk now or c) as and when you interact with out of date information. In this way, 
rdhs hopes to allow studies and analysis of DHS data to both be easier, and reproducible allowing
...
website that will allow you to leverage more API call returns, and will be stored within
...
within your user cache directory for your operating system. E.g:
...
perhaps antimalarial prescription has decreased after RDTs were introduced (assumed 2010). So we can query 
for that by specifying the "data" endpoint, and using suitable filter queries provided as a list as follows:


```{r API call, include=TRUE, message = FALSE, warning = FALSE,fig.height=7,fig.width=7}
# Make an api request
...
somewhat nicely presented on the page for you to see way:
...
are returned. If you want to return fewer than all the results, or look at the actual API response
...
str(available_surveys,vec.len=2)
```
...
attributes. At the moment, .dta datasets are read in using \code{foreign::read_dta}. This will change at a later 
date to allow users to specif which fun
...
At the moment this will indiscriminately download the zip file and unpack it to a directory within
...
survey, e.g. birth recode, individual recode, stored within that same directory. The downloads object
will now be a list of file paths to where the files were saved to. Depending on the download option
this will either be the zip file, the extracted files from the zip, or the rds object created by unzipping
the zip file and reading in the approporiate dataset. 
...
what their description is. If you do not wish to have this information then change the `reformat` argument to `FALSE`.
...
head(downloaded_dataset$Survey_Code_Descriptions)

# and let's have a look at how the factor reformat has been helpful
downloaded_dataset$Survey_Code_Descriptions[downloaded_dataset$Survey_Code_Descriptions$Code=="ML101",]
...
we now want to know which survey codes/questions are contained in each survey that we want in our final extracted
...
to filter by just choosing the codes you want from one survey. Similarly, the questions for the same code may be subtly
...
So let's now have a look at the surveys we downloaded, and query for survey questions that included anything about fevers,
malaria, or tests. 

(At the moment the search terms are just combined to produce an OR regex query. )
...
questions data.frame as needed or directly pass this on to extract the relevant data.

```{r Survey question extraction, include=TRUE, message = FALSE, warning = FALSE}
# grab a subset of the questions to save time
subset_quest <- sample(dim(questions)[1],3,replace=FALSE)

# extract the questions, and filter by search terms.
extracted_data <- client$extract(questions[subset_quest,])

# and the structure of the results. 
str(extracted_data)
...
The last section is an open ended question still, so any ideas are appreciated. It would be nice to be able to
chain the following 7 steps somehow so that the pipeline could then itself be cached so that others could recreate
