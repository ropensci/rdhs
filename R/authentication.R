...
##' @param your_project Character string for the name of your project that
##'   gives you access to the DHS database
...
available_datasets <- function(your_email, your_password, your_project,
  ...
  # remove any missing matches (souldn't happen)
  ...
  if (length(missings) > 0) {
    missing_country_codes <- unique(res$FileName[missings] %>%
                                      substr(start = 1, stop = 2))
    call <- surveys_api_results
    res$SurveyNum[missings] <- qdapRegex::rm_between(
      res$URLS[missings], "surv_id=", "&dm", extract = TRUE
    ) %>% unlist()

    missing_survey_nums <- unique(res$SurveyNum[missings])

    ## TODO: MAKE THIS LESS CUSTOM BUILT  - also there is no telling that
    ## the surveys api is any better for providing information
    needed_cols <- c("SurveyId", "SurveyYearLabel", "SurveyType",
    ...
    filetype_stems <- lapply(file_types, function(x) {
      unique(toupper(substr(res$FileName[res$FileType == x], 3, 4)))
    }) %>% unlist()

    file_formats <- unique(res$FileFormat)
    file_formats <- file_formats[-which(file_formats == "")]
    fileformat_endings <- lapply(file_formats, function(x) {
      unique(toupper(substr(res$FileName[res$FileFormat == x], 7, 8)))
    }) %>% unlist()
    ...
    for (i in missing_survey_nums) {
      ...
      if (length(which(call$SurveyNum == i)) > 0) {
        res[missing_pos, needed_cols] <- call[which(call$SurveyNum == i),
                                              needed_cols]
      }

      # fill in file type and format from filename
      st <- toupper(substr(res$FileName[missing_pos], 3, 4))
      res$FileType[missing_pos] <- file_types[match(st, filetype_stems)]
      res$FileFormat[missing_pos] <- file_formats[match(st, fileformat_endings)]
    }
  }
...
##' @param your_email Character for email address for DHS website
##' @param your_password Character for password for DHS website
##' @param your_project Character string for the name of your project that gives
##'   you access to the DHS database
...
download_datasets <- function(desired_dataset,
  ...
  if (download_option >= 2) {
    ...
    if (!is.character(res)) {
      ...
      # if the class of the object is from a geo file then we
