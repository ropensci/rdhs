...
test_that("avaialble surveys and download work", {
  ...
  sen <- cli$download_datasets("SNGE71FL.ZIP",download_option = "z")
  ...
  downloads <- cli$download_datasets(dataset_filenames = "AOBR62DT.ZIP",download_option = "r",mode="raw")
  ...
  downloads <- cli$download_datasets(dataset_filenames = "AOBR62DT.ZIP",download_option = "r",mode="raw",reformat=TRUE)

  # remove the cache and then check for the toupper no reformat
  cli$.__enclos_env__$private$storr$del(key="AO2011MIS_AOBR62DT_rds_TRUE",namespace = "downloaded_datasets")
  downloads <- cli$download_datasets(dataset_filenames = "AOBR62DT.ZIP",download_option = "r",mode="raw",all_lower=FALSE)

  # remove the cache and then check for the reformat to upper
  cli$.__enclos_env__$private$storr$del(key="AO2011MIS_AOBR62DT_rds_TRUE",namespace = "downloaded_datasets")
  downloads <- cli$download_datasets(dataset_filenames = "AOBR62DT.ZIP",download_option = "r",mode="raw",reformat=TRUE,all_lower=TRUE)

  # check FL with upper foreign only
  downloads <- cli$download_datasets(dataset_filenames = "AOBR62DT.ZIP",download_option = "r",mode="raw")

  # check hierarchal catch and sas7bdat
  downloads <- cli$download_datasets(dataset_filenames = "AOIR62.ZIP",download_option = "r",mode="raw")
  expect_identical(downloads[[1]],"No support for importing hierarchal .dat")

  downloads <- cli$download_datasets(dataset_filenames = "AOHR51sd.zip",download_option = "r",mode="raw")
